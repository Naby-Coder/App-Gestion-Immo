/*
  ...
  1. Sample Agents
    - Insert agents with conflict handling
    - Each agent has bio, position, and specialties
  
  2. Sample Properties
    - Insert properties linked to agents
    - Various types: Villa, Apartment, Commercial, Office
    - Mix of sale and rental properties
    - Include features, images, and location data

  3. Conflict Handling
    - Use ON CONFLICT DO NOTHING to avoid duplicate key errors
    - Ensures migration can be run multiple times safely
*/

-- Insert sample agents (with conflict handling)
...
-- Insert sample properties (with conflict handling)
